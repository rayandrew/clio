...
// g++ -std=c++17 concept_finder.cpp
// ./a.out ./test ./runs/raw/tencent/characteristic/1063/1m/characteristic.csv iops
...
std::map<int, int> calc_mode_row(const std::vector<Row> &rows, const std::string &key)
{
...
void concept_finder(const fs::path &out_dir, const fs::path &characteristic_file, const std::string &metric = "iops")
{
    ...
    fs::path output_path = out_dir / metric;

    fs::create_directories(output_path);

    std::vector<Row> rows = read_csv(characteristic_file, metric);
    std::cout << "Read " << rows.size() << " rows" << std::endl;
    int index = 0;
    for (auto &row : rows)
    {
        // std::cout << "Processing row " << row.index << std::endl;
        row.group = determine_group(row.value, GROUP_THRESHOLD, GROUP_OFFSET);
        int last_window = std::max(0, index - ROLLING_WINDOW);
        std::vector<Row> prev_windows(rows.begin() + last_window, rows.begin() + index);

        // std::vector<Row> prev_windows(rows.end() - std::min(ROLLING_WINDOW, static_cast<int>(rows.size())), rows.end());
        prev_windows.push_back(row);
        row.mode = calc_mode_row(prev_windows, "group");
    ...
    for (int i = 1; i < num_data; ++i)
    {
        ...
        std::vector<Row> next_windows(rows.begin() + i, rows.begin() + std::min(num_data, i + ROLLING_WINDOW));
        row.next_mode = calc_mode_row(next_windows, "group");
        ...
        else
        {
            ...
            else
            {
                ...
                std::map<int, int> potential_modes = get_similar_modes(row.mode, row.group, 2, false);
...
int main(int argc, char *argv[])
{
    if (argc < 3)
    {
        std::cerr << "Usage: " << argv[0] << " <output_directory> <characteristic_file> [metric]\n";
        return 1;
    }
