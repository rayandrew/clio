...
from snakemake.common import ON_WINDOWS, async_run, get_function_params, get_input_function_aux_params, is_namedtuple_instance
from snakemake.exceptions import MissingOutputException, WildcardError, WorkflowError
...
from snakemake_interface_storage_plugins.io import WILDCARD_REGEX, IOCacheStorageInterface, Mtime, get_constant_prefix


def lutime(file, times):
    success = lutime_raw(file, times)
    if not success:
        logger.warning("Unable to set mtime without following symlink because it seems " "unsupported by your system. Proceeding without.")
...
class IOCache(IOCacheStorageInterface):
    ...
    async def mtime_inventory(self, jobs, n_workers=8):
        ...
        tasks = [asyncio.get_event_loop().create_task(worker(queue)) for _ in range(n_workers)]
...
class _IOFile(str, AnnotatedStringInterface):
    ...
    def __new__(cls, file):
        ...
        is_callable = isfunction(file) or ismethod(file) or (is_annotated and bool(file.callable))
    ...
    def check(self):
        hint = "It can also lead to inconsistent results of the file-matching " "approach used by Snakemake."
        ...
        if self._file.startswith(" "):
            logger.warning("File path '{}' starts with whitespace. " "This is likely unintended. {}".format(self._file, hint))
        if self._file.endswith(" "):
            logger.warning("File path '{}' ends with whitespace. " "This is likely unintended. {}".format(self._file, hint))
        if "\n" in self._file:
            logger.warning("File path '{}' contains line break. " "This is likely unintended. {}".format(self._file, hint))
        if _double_slash_regex.search(self._file) is not None and not self.is_storage:
            logger.warning("File path {} contains double '{}'. " "This is likely unintended. {}".format(self._file, os.path.sep, hint))
    ...
    async def protected(self):
        ...
        return await self.exists_local() and not os.access(self.file, os.W_OK) and not os.path.islink(self.file)
    ...
    async def mtime_uncached(self, skip_storage: bool = False):
        ...
        location.
        """
        mtime_in_storage = (await self.storage_object.managed_mtime()) if self.is_storage and not skip_storage else None
        ...
        try:
            ...
            raise WorkflowError("Unable to obtain modification time of file {} because of missing " "read permissions.".format(self.file))
    ...
    async def is_checksum_eligible(self):
        return await self.exists_local() and not os.path.isdir(self.file) and await self.size() < 100000 and not self.is_fifo()
        ...
        if not await self.exists_local():
            try:
                if os.lstat(self.file):
                    raise WorkflowError(f"File {self.file} seems to be a broken symlink.")
    ...
    def protect(self):
        mode = os.lstat(self.file).st_mode & ~stat.S_IWUSR & ~stat.S_IWGRP & ~stat.S_IWOTH
        ...
        try:
            ...
            if e.errno == 2:
                raise MissingOutputException(
                    "Output file {} of rule {} shall be touched but " "does not exist.".format(self.file, self.rule.name),
    ...
    def clone_flags(self, other, skip_storage_object=False):
        ...
        if isinstance(other._file, AnnotatedString) or isinstance(other._file, _IOFile):
            ...
            if skip_storage_object and self.is_storage:
                ...
                assert "storage_object" not in self._file.flags, "bug: storage object cannot be safely cloned as the query might have " "to change"

    def clone_storage_object(self, other):
        if isinstance(other._file, AnnotatedString) and "storage_object" in other._file.flags:
            self._file.flags["storage_object"] = copy.copy(other._file.flags["storage_object"])
...
def is_callable(value: Any):
    return callable(value) or (isinstance(value, AnnotatedStringInterface) and value.is_callable())


_double_slash_regex = re.compile(r"([^:]//|^//)") if os.path.sep == "/" else re.compile(r"\\\\")


async def wait_for_files(files, latency_wait=3, wait_for_local=False, ignore_pipe_or_service=False):
    ...
    async def get_missing():
        return [
            ...
            if not (
                ...
                if (isinstance(f, _IOFile) and f.is_storage and (not wait_for_local or f.should_not_be_retrieved_from_storage))
                else os.path.exists(f) if not ((is_flagged(f, "pipe") or is_flagged(f, "service")) and ignore_pipe_or_service) else True
            )
        ]
...
def regex_from_filepattern(filepattern):
    ...
    for match in WILDCARD_REGEX.finditer(filepattern):
        ...
        if wildcard in wildcards:
            if match.group("constraint"):
                raise ValueError("Constraint regex must be defined only in the first " "occurrence of the wildcard in a string.")
...
def is_callable(value):
    return callable(value) or (isinstance(value, _IOFile) and value._is_function) or (isinstance(value, AnnotatedString) and value.callable is not None)
...
class QueueInfo:
    ...
    def consume(self, wildcards):
        assert self.finished is False, "bug: queue marked as finished but consume method called again"
...
def sourcecache_entry(value, orig_path_or_uri):
    ...
    assert not isinstance(orig_path_or_uri, SourceFile), "bug: sourcecache_entry should receive a path or uri, not a SourceFile"
...
def expand(*args, **wildcard_values):
    ...
    **wildcard_values -- the wildcards as keyword arguments
    ...
    wildcard_values = {
        filepattern: {k: v for k, v in wildcard_values.items() if k in re.findall(r"{([^}\.[!:]+)", filepattern)} for filepattern in filepatterns
    }

    def do_expand(wildcard_values):
        def flatten(wildcard_values):
            for wildcard, values in wildcard_values.items():
                if isinstance(values, str) or not isinstance(values, collections.abc.Iterable) or is_namedtuple_instance(values):
                    values = [values]
...
def multiext(prefix, *extensions):
    ...
    if any((r"/" in ext or r"\\" in ext) for ext in extensions):
        raise WorkflowError(r"Extensions for multiext may not contain path delimiters (/,\).")
...
def limit(pattern, **wildcards):
    ...
    **wildcards -- the wildcards as keyword arguments
    ...
    """
    return pattern.format(**{wildcard: "{{{},{}}}".format(wildcard, "|".join(values)) for wildcard, values in wildcards.items()})


def _get_prefix(pattern: str) -> Tuple[str, str]:
...
def glob_wildcards(pattern, files=None, followlinks=False):
    ...
    if is_flagged(pattern, "storage_object"):
        if files is not None:
            raise WorkflowError("Error in glob_wildcards(): the files argument may not " "be combined with a storage object as pattern.")
        ...
        _, pattern = _get_prefix(pattern)
        # pattern = fnmatch.translate(pattern)
        pattern = pattern.replace("*", r"(.*)")
    else:
        pattern = os.path.normpath(str(pattern))
...
class Namedlist(list):
        ...
        if toclone is not None:
            if custom_map is not None:
                ...
                self.extend(
                    ...
                    (str(x) if x.storage_object.retrieve else x.storage_object.query) if isinstance(x, _IOFile) and x.storage_object is not None else str(x)
                    for x in toclone
                )
    ...
    def _used_attribute(*args, _name, **kwargs):
        ...
        """
        raise AttributeError("{_name}() cannot be used; attribute name reserved" " for use in some existing workflows".format(_name=_name))
    ...
    def _set_name(self, name, index, end=None):
        ...
        if name not in self._allowed_overrides and hasattr(self.__class__, name):
            raise AttributeError("invalid name for input, output, wildcard, " "params or log: {name} is reserved for internal use".format(name=name))
...
class PeriodicityDetector:
    ...
    def is_periodic(self, value):
        ...
        while value[-pos] != last_letter:  # as long as last letter is not seen, repeat length is minimally pos
            if len(value) < (pos * self.min_repeat) or counter[value[-pos]] < self.min_repeat:
                return None
            pos += 1
