...
class FindMinTimestampReducer {
 public:
  ...
  FindMinTimestampReducer(const std::vector<fs::path>& paths, P* pbar = nullptr)
      : paths(paths), min_ts{std::numeric_limits<double>::max()}, pbar{pbar} {
    static_assert(sizeof...(Dummy) == 0, "Do not specify template arguments!");
  }

  template <typename... Dummy, typename P = ProgressBar, std::enable_if_t<!is_progress_bar_s<P>::value, bool> = true>
  FindMinTimestampReducer(const std::vector<fs::path>& paths)
      : paths(paths), min_ts{std::numeric_limits<double>::max()} {
    static_assert(sizeof...(Dummy) == 0, "Do not specify template arguments!");
  }

  template <typename... Dummy, typename P = ProgressBar, std::enable_if_t<is_progress_bar_s<P>::value, bool> = true>
  FindMinTimestampReducer(FindMinTimestampReducer& x, oneapi::tbb::split)
      : paths(x.paths), min_ts(std::numeric_limits<double>::max()), pbar{x.pbar} {
    static_assert(sizeof...(Dummy) == 0, "Do not specify template arguments!");
  }
...
template <typename Trace, typename ProgressBar = void>
class FindMinMaxTimestampReducer {
 public:
  template <typename... Dummy, typename P = ProgressBar, std::enable_if_t<is_progress_bar_s<P>::value, bool> = true>
  FindMinMaxTimestampReducer(const std::vector<fs::path>& paths, P* pbar = nullptr)
      : paths(paths),
        min_ts{std::numeric_limits<double>::max()},
        max_ts{std::numeric_limits<double>::min()},
        pbar{pbar} {
    static_assert(sizeof...(Dummy) == 0, "Do not specify template arguments!");
  }

  template <typename... Dummy, typename P = ProgressBar, std::enable_if_t<!is_progress_bar_s<P>::value, bool> = true>
  FindMinMaxTimestampReducer(const std::vector<fs::path>& paths)
      : paths(paths), min_ts{std::numeric_limits<double>::max()}, max_ts{std::numeric_limits<double>::min()} {
    static_assert(sizeof...(Dummy) == 0, "Do not specify template arguments!");
  }

  template <typename... Dummy, typename P = ProgressBar, std::enable_if_t<is_progress_bar_s<P>::value, bool> = true>
  FindMinMaxTimestampReducer(FindMinMaxTimestampReducer& x, oneapi::tbb::split)
      : paths(x.paths),
        min_ts(std::numeric_limits<double>::max()),
        max_ts(std::numeric_limits<double>::min()),
        pbar{x.pbar} {
  ...
  template <typename... Dummy, typename P = ProgressBar, std::enable_if_t<!is_progress_bar_s<P>::value, bool> = true>
  FindMinMaxTimestampReducer(FindMinMaxTimestampReducer& x, oneapi::tbb::split)
      : paths(x.paths), min_ts(std::numeric_limits<double>::max()), max_ts(std::numeric_limits<double>::min()) {
    static_assert(sizeof...(Dummy) == 0, "Do not specify template arguments!");
  }

  void join(const FindMinMaxTimestampReducer& y) {
    if (y.min_ts < min_ts) {
      min_ts = y.min_ts;
    }
    if (y.max_ts > max_ts) {
  ...
  void operator()(const oneapi::tbb::blocked_range<std::size_t>& r) {
    const auto& paths = this->paths;
    double      min_ts = this->min_ts;
    double      max_ts = this->max_ts;

    for (std::size_t i = r.begin(); i != r.end(); ++i) {
      auto [trace_min_ts, trace_max_ts] = do_work(paths[i]);
      if (pbar)
        pbar->tick();
      if (min_ts > trace_min_ts)
        min_ts = trace_min_ts;
      if (max_ts < trace_max_ts)
        max_ts = trace_max_ts;
    }
    this->min_ts = min_ts;
    this->max_ts = max_ts;
  ...
  inline decltype(Trace::Entry::timestamp) get_min() const { return min_ts; }
  inline decltype(Trace::Entry::timestamp) get_max() const { return max_ts; }

  std::tuple<decltype(Trace::Entry::timestamp), decltype(Trace::Entry::timestamp)> get() const {
    return {min_ts, max_ts};
  }

 private:
  std::tuple<double, double> do_work(const fs::path& path) {
    double trace_start_time = std::numeric_limits<double>::max();
    double trace_end_time = std::numeric_limits<double>::min();
    Trace  trace(path);
    trace.stream([&](const auto& item) {
      if (item.timestamp < trace_start_time) {
        trace_start_time = item.timestamp;
      }
      if (item.timestamp > trace_end_time) {
        trace_end_time = item.timestamp;
      }
    });
    return {trace_start_time, trace_end_time};
  }

 private:
  std::vector<fs::path>             paths;
  decltype(Trace::Entry::timestamp) min_ts;
  decltype(Trace::Entry::timestamp) max_ts;
  ProgressBar*                      pbar;
};

template <typename Trace, typename ProgressBar>
auto get_min_max_timestamp(const std::vector<fs::path>& paths, ProgressBar& pbar) {
  static_assert(is_progress_bar<ProgressBar>(), "pbar needs to have `tick` function!");
  FindMinMaxTimestampReducer<Trace, ProgressBar> r(paths, &pbar);
  oneapi::tbb::parallel_reduce(oneapi::tbb::blocked_range<size_t>(0, paths.size()), r);
  return r.get();
}

template <typename Trace>
auto get_min_max_timestamp(const std::vector<fs::path>& paths) {
  FindMinMaxTimestampReducer<Trace, void> r(paths);
  oneapi::tbb::parallel_reduce(oneapi::tbb::blocked_range<size_t>(0, paths.size()), r);
  return r.get();
}
