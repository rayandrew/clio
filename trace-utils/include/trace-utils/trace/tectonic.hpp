...
class TectonicTrace : public Trace<tectonic::Entry> {
 public:
  ...
  virtual void raw_stream_column(const fs::path& path, unsigned int column, RawReadColumnFn&& read_fn) const override;
};
}  // namespace trace_utils::trace
...
class formatter<trace_utils::trace::TectonicTrace::Entry> {
 public:
  ...
  constexpr auto format(trace_utils::trace::TectonicTrace::Entry const& entry, FmtContext& ctx) const
      -> format_context::iterator {
    return format_to(ctx.out(), "{{timestamp={}, offset={}, size={}, read={}, volume={}}}", entry.timestamp,
                     entry.offset, entry.length, entry.opcode, entry.device_id);
  }
};
}  // namespace fmt
