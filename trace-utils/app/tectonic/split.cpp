...
namespace trace_utils::app::tectonic {
namespace split {
const char* name = "split";
const char* description = "tectonic Split";
}  // namespace split
...
void SplitApp::run([[maybe_unused]] CLI::App* app) {
  ...
  using Map = oneapi::tbb::concurrent_hash_map<std::size_t, std::tuple<Mutex, MapV, std::atomic_size_t>>;
  ...
  utils::f_sec dur = utils::get_time([&] {
    ...
    auto [min_time, max_time] = utils::get_min_max_timestamp<trace::TectonicTrace>(paths, pbar);
    trace_start_time = min_time;
    trace_end_time = max_time;
  ...
  if (trace_end_time == std::numeric_limits<double>::min()) {
    throw Exception("Cannot find max ts");
  }

  log()->info("Finding min max stamp takes {}", std::chrono::duration_cast<std::chrono::milliseconds>(dur));
  log()->info("Min timestamp = {}", trace_start_time);
  log()->info("Max timestamp = {}", trace_end_time);
  ...
  dur = utils::get_time([&] {
    ...
    oneapi::tbb::parallel_for_each(paths.cbegin(), paths.cend(), [&](const auto& path) {
      trace::TectonicTrace trace(path);
  ...
  dur = utils::get_time([&] {
    ...
    oneapi::tbb::parallel_for_each(unsorted_temp_paths.cbegin(), unsorted_temp_paths.cend(), [&](const auto& path) {
      ...
      std::sort(vecs.begin(), vecs.end(), [](const auto& a, const auto& b) { return a.timestamp < b.timestamp; });
