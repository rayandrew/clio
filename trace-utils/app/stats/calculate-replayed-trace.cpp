...
{
    namespace replayed_trace
    {
        const char *name = "replayed-trace";
        const char *description = "Calculate replayed Trace Stats";
    } // namespace calculate_replayed_trace

    CalculateReplayedTraceApp::CalculateReplayedTraceApp() : App(replayed_trace::name, replayed_trace::description)
    {
    }

    CalculateReplayedTraceApp::~CalculateReplayedTraceApp()
    {
        indicators::show_console_cursor(true);
    }

    void CalculateReplayedTraceApp::setup_args(CLI::App *app)
    {
    ...
    void CalculateReplayedTraceApp::setup()
    {
    ...
    void CalculateReplayedTraceApp::run([[maybe_unused]] CLI::App *app)
    {
        ...
        std::vector<TraceCombiner<trace::ReplayedTrace>> traces;
        ...
        std::vector<trace::ReplayedTrace> temp_traces;
        for (std::size_t i = 1; i < paths.size() + 1; ++i)
        {
            ...
            if ((i % num_chunk) == 0)
            {
                ...
                temp_traces.clear();
                std::vector<trace::ReplayedTrace>().swap(temp_traces);
            }
        }
        ...
        oneapi::tbb::parallel_for(oneapi::tbb::blocked_range<size_t>(0, traces.size()),
            ...
            for (std::size_t chunk = r.begin(); chunk < r.end(); ++chunk) {
                try {
                    ...
                    auto characteristic = ReplayedCharacteristic::from(trace, true);
                    if (i == 0)
                    {
