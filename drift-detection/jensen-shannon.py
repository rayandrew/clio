...
def process_preds(feature, preds):
    ...
    preds = pd.DataFrame(preds, columns=[f'{feature}_js_distance'])
    preds[f'{feature}_js_label'] = 0
    sorted_df = preds.sort_values(by=f'{feature}_js_distance', ascending=False)
    lowest_ten_indices = sorted_df.head(10).index
    highest_ten_indices = sorted_df.tail(10).index
    preds.loc[lowest_ten_indices, f'{feature}_js_label'] = 1
    preds.loc[highest_ten_indices, f'{feature}_js_label'] = 2
    
    # Counting the number of labels for KS
    label_counts = preds[f'{feature}_js_label'].value_counts()
    print(f"jensen-shannon Labels for {feature}:")
    print(label_counts)
...
def moving_prediction(args, feature):
    ...
    while end < len(df_data):
        ...
        js_dist = distance.jensenshannon(data_ref, x)
        
        preds.append(js_dist)
...
def main(args):
    ...
    if args.preprocess == 'none':
        filename = filename + '_jensen-shannon' + '.csv'
    else:
        filename = filename + '_jensen-shannon' + '_' + args.preprocess + '.csv'
