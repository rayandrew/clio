...
def calc_trapezoid_area(len1: float, len2: float, h: float) -> float:
    return (len1 + len2) * h / 2


def calc_area_going_right_v0(y_idx: int, x_values: list[int], y_values: list[int]) -> int:
    sum_area = 0
    n = len(x_values)
...
def calc_area_going_down_v0(y_idx: int, x_values: list[int], y_values: list[int]) -> int:
    sum_area = 0
    n = len(y_values)
...
def calc_area_going_left(lat_array: list[int], ip_latency_percent: int) -> int:
    sum_area = 0
...
def area_based(arr_x_value: list[int], x_max: int = None) -> tuple[int, int]:
...
__all__ = ["tangent_based", "area_based"]
