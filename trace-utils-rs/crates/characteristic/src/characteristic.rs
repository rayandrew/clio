...
impl TryInto<RawTraceCharacteristic> for Vec<MsftTrace> {
    ...
    fn try_into(self) -> Result<RawTraceCharacteristic, Box<dyn std::error::Error>> {
        ...
        let mut offset: Vec<OrderedFloat<f64>> = Vec::new();
        let mut size: Vec<OrderedFloat<f64>> = Vec::new();
        let mut read_size: Vec<OrderedFloat<f64>> = Vec::new();
        let mut write_size: Vec<OrderedFloat<f64>> = Vec::new();
        ...
        for msft in self.iter() {
            let msft_timestamp: f64 = msft.timestamp.into();
            if num_io == 0 {
                start_time = msft_timestamp;
            }

            num_io += 1;
            end_time = msft_timestamp;
            iat.push(OrderedFloat::from(msft_timestamp - last_time));
            offset.push(OrderedFloat::from(msft.offset as f64));
            size.push(OrderedFloat::from(msft.size as f64));

            if msft.read {
                read_count += 1;
                read_iat.push(OrderedFloat::from(msft_timestamp - last_read_time));
                read_size.push(OrderedFloat::from(msft.size as f64));
                last_read_time = msft_timestamp;
            } else {
                write_count += 1;
                write_iat.push(OrderedFloat::from(msft_timestamp - last_write_time));
                write_size.push(OrderedFloat::from(msft.size as f64));
                last_write_time = msft_timestamp;
            }

            last_time = msft_timestamp;
        }
